module Jekyll
  ...
  class CategoryPages < Generator
    ...
    def paginate(site, page)
      ...
      author_posts = site.config['authors'][page.data['author']].sort_by { |p| -p.date.to_f }
 
      # calculate total number of pages
      pages = CategoryPager.calculate_pages(category_posts, site.config['cutsom_paginate'].to_i)
  ...
  class CategoryPager < Pager
    ...
    def self.pagination_enabled?(config, page)
      page.name == 'index.html' && page.data.key?('category') && !config['cutsom_paginate'].nil?
    end
