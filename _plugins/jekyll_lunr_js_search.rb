...
module Jekyll
  ...
  class SearchEntry
    ...
    def self.create_from_page(page, renderer)
      title, url, image, author, addressee, position = extract_title_and_url(page)
      body = renderer.render(page)
      date = nil
      categories = []
      tags = []
      
      SearchEntry.new(title, url, date, categories, tags, image, body, author, addressee, position)
    end
    
    def self.create_from_post(post, renderer)
      title, url, image, author, addressee, position = extract_title_and_url(post)
    ...
    def self.extract_title_and_url(item)
      data = item.to_liquid
      [ data['title'], data['url'], data['image'], data['author'], data['addressee'], data['position'] ]
    end

    attr_reader :title, :url, :date, :categories, :tags, :image, :body, :author, :addressee, :position
    
    def initialize(title, url, date, categories, tags, image, body, author, addressee, position)
      @title, @url, @date, @categories, @tags, @image, @body, @author, @addressee, @position = title, url, date, categories, tags, image, body, author, addressee, position
    end
